...
var BZCompLkp = {
  LkpDataGrid: function () {
    ...
    this.Load = function () {
      ...
      //EDIT BUTTON
      if (
        this.Config.FormButtons.indexOf("Select") > -1) {
          debugger;
        var EditBtnJson = {
          data: {text: this.Config.DataText, value: this.Config.DataValue},
          title: "Action",
          render: function (data) {
            var EditButton = "";
            var ViewButton = "";
            if (config.FormButtons.indexOf("Select") > -1) {
              var ViewButton =
                '<a href="#" class="tx-primary" onclick="Lkp_OnSelect(\'' + data.Text + '\',\'' + data.Value + '\')">Select</a>';
            }         
